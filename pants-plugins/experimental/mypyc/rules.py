...
async def generate_setup_py(request: MyPycGenerateSetupPyRequest) -> GeneratedSetupPy:
    ...
    logger.info("Inside the custom generate_setup_py function")
...
async def mypyc_setup_kwargs(request: MyPycSetupKwargsRequest) -> SetupKwargs:
    logger.debug(f"This is the requested target: {request.target}")
    transitive_targets = await Get(
        TransitiveTargets,
        TransitiveTargetsRequest([request.target.address]),
    )

    logger.debug(f"These are the transitive targets: {transitive_targets}")
    python_source_files = (
        await Get(
            PythonSourceFiles,
            PythonSourceFilesRequest(
    ...
    source_files = list(python_source_files[0].source_files.snapshot.files)
    logger.debug(f"Here are the source files {source_files}")

    # TODO: mypycify needs to be a function in the setup.py - not buried in a string
    # Alternatively, could try to run it here and pass the result into ext_modules, but that's going to start the compilation here too I think
    kwargs = SetupKwargs(
        {
            **request.explicit_kwargs,
            "ext_modules": f"mypycify({source_files})",
        },
        address=request.target.address,
    )
    logger.info(f"These are the new SetupKwargs: {kwargs.kwargs}")
    return kwargs
